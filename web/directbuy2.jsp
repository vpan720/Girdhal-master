...
<a href="index.html">Logout</a>
    ...
    <body>
        ...
        <c:if test="${empty param.c3}">
            <c:redirect url="product.jsp">
                <c:param name="errMsg7" value="Please Select Checkbox"/>
            </c:redirect>
        </c:if>
        <c:if test="${empty param.qty3}">
            <c:redirect url="product.jsp">
                <c:param name="errMsg8" value="Please Enter Quantity"/>
            </c:redirect>
        </c:if>
        <c:if test="${param.qty3<10}">
            <c:redirect url="product.jsp">
                <c:param name="errMsg9" value="Please Enter Valid Quantity"/>
            </c:redirect>
        </c:if>
        <sql:update dataSource="${dbsource}" var="result">
            insert into cart(id,qty) values(?,?);
            <sql:param value="${param.c3}"/>
            <sql:param value="${param.qty3}"/>
        </sql:update>
            <c:if test="${result>=1}">
